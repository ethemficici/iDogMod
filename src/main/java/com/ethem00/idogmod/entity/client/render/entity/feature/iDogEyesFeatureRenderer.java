...
@OnlyIn(Dist.CLIENT)
public class iDogEyesFeatureRenderer<T extends iDogEntity, M extends iDogEntityModel<T>> extends EyesLayer<T, M> implements iDogEyeVariants {
    private static final RenderType DEFAULT_EYES_RENDERLAYER = RenderType.eyes(ResourceLocation.fromNamespaceAndPath(iDogMod.MOD_ID, "textures/entity/idog/eyes/idog_eyes_5.png"));

    public iDogEyesFeatureRenderer(RenderLayerParent<T, M> featureRendererContext) {
    ...
    private ResourceLocation getEyesIdentifier(String disc, Integer variant) {
    ...
    public void render(PoseStack poseStack, MultiBufferSource buffer, int packedLight, T iDog,
        ...
        RenderType eyeLayer = RenderType.entityTranslucentEmissive(getEyesIdentifier(iDog.getCurrentDisc(), iDog.getEyeVariant()));
        VertexConsumer vc = buffer.getBuffer(eyeLayer);

        this.getParentModel().renderToBuffer(poseStack, vc, 15728640, OverlayTexture.NO_OVERLAY,
                1.0F, 1.0F, 1.0F, iDog.getEyeAlpha());
    }
}
